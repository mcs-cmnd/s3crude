...
if ($downloadFile) {
    $response = Invoke-RestMethod -SkipCertificateCheck -Uri $requestUrl -Method $requestType -Headers $headers -ContentType $contentType -OutFile $downloadFile
} elseif ($uploadFile) {
    $response = Invoke-RestMethod -SkipCertificateCheck -Uri $requestUrl -Method $requestType -Headers $headers -ContentType $contentType -InFile $uploadFile
} elseif ($xmlBody) {
    $response = Invoke-RestMethod -SkipCertificateCheck -Uri $requestUrl -Method $requestType -Headers $headers -ContentType $contentType -Body $xmlBody
}    
else {
    $response = Invoke-RestMethod -SkipCertificateCheck -Uri $requestUrl -Method $requestType -Headers $headers -ContentType $contentType
}
